...
if (-not (Test-Admin)) {
    ...
    # Always relaunch the 64-bit PowerShell. If we're running 32-bit (e.g. the
    # user opened "Windows PowerShell (x86)"), System32 is filesystem-redirected
    # to SysWOW64; Sysnative is the alias that reaches the real 64-bit System32.
    # Running 64-bit matters so HKLM reads hit the same registry view the 64-bit
    # MSI writes to.
...
# Compute size in megabytes WITHOUT the 1MB literal: PS 5.1's parser refuses
# the 1MB suffix when it appears inside a complex "$($expr)" interpolation.
$mb = [math]::Round($msiAsset.size / 1048576, 1)
Step ("Downloading {0} ({1} MB)" -f $msiAsset.name, $mb)
