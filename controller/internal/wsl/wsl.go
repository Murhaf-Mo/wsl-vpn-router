...
func Run(distro, script string) (string, error) {
	cmd := exec.Command("wsl.exe", "-d", distro, "-e", "bash", "-c", script)
...
func WSLPath(distro, winPath string) (string, error) {
	cmd := exec.Command("wsl.exe", "-d", distro, "-e", "wslpath", "-a", winPath)
	cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
	out, err := cmd.Output()
	if err != nil {
		return "", err
...
func isWslProcess(pid int) bool {
	out, err := exec.Command("tasklist.exe", "/FI", "PID eq "+strconv.Itoa(pid), "/FO", "CSV", "/NH").Output()
	if err != nil {
		return false
	}
...
// IsInstalled reports whether the MSI is installed (HKLM key present). In dev
// mode (running vpn.exe from a checkout), this returns false and the install +
// data layout collapses to a single directory — preserving the pre-1.0 single-
// directory project layout for hacking on the controller without an MSI build.
func IsInstalled() bool {
	...
	defer k.Close()
	v, _, err := k.GetStringValue(AppKeyValue)
	return err == nil && v != ""
}
