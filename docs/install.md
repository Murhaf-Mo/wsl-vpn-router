...
5. **`wslprovision.ps1`** runs directly in the same console — you see DISM, `wsl --update`, `wsl --install -d Ubuntu`, and apt output as it happens. Enables WSL features if needed, installs Ubuntu if
...
|------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------|
| `C:\Program Files\wsl-vpn-router\` | `vpn.exe`, `vpn-tray.exe`, `vpnctl.py`, `static/`, `setup-wsl.sh`, `wslprovision.ps1`, `uninstall.ps1`, the default `vpn.list` / `direct.list` shipped with the release | Admin only    |
