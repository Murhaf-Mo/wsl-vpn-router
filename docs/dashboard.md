...
All endpoints are unauthenticated. They bind to `0.0.0.0` of the WSL VM, which is reachable from your Windows host. Other machines on your LAN can also reach them if they know the WSL VM's IP — but the WSL VM's IP usually isn't routable from outside your machine.
