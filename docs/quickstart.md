...
If your VPN sends a **Duo push**, approve it on your phone now. If it asks for a passcode or a specific second-factor word in a separate prompt, you'd put that in `[vpn] mfa_response` in
`config.toml` — see [configuration.md](configuration.md#vpn-section).
