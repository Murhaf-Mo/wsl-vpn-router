...
If you don't want a particular release, just don't click the tray notification. Next cycle it'll still see "1.2.0 newer than 1.1.0" and re-show the prompt. There's no per-release "skip" — the tool's state is just "what version am I" vs "what's latest". If you want to pin to a specific older version, install that version's MSI directly from the [GitHub releases page](https://github.com/Murhaf-Mo/wsl-vpn-router/releases) and don't run `vpn upgrade`.
