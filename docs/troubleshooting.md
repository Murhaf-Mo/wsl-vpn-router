...
Microsoft Store throttling. `wslprovision.cmd` uses `--web-download` to bypass the Store, but the Store is still consulted for kernel updates. Retry after a few minutes, or install Ubuntu manually:
...
If that returns NXDOMAIN, fix `[vpn] host` in config.toml. If it returns an IP, but `oc.log` still says getaddrinfo failed, your WSL distro's `/etc/resolv.conf` is broken — check with `wsl -d Ubuntu -- cat /etc/resolv.conf`.
...
For JetBrains IDEs: **Settings → Appearance & Behavior → System Settings → HTTP Proxy** — set to "Auto-detect proxy settings" (which reads PAC) **or** "Manual proxy" pointing at the same URL as `HTTP_PROXY`.
...
Either `vpn.exe` has a bug or `cmd.exe` isn't on the PATH the tray inherited. Make sure the tray was launched via `vpn-tray.exe` from Program Files (or via the autostart Run key), not via some custom launcher with a clobbered PATH.
