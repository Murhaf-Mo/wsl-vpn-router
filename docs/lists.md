...
`example.com` matches `example.com` and `mail.example.com`. To match **only** `example.com` and nothing else, you can't — PAC's `dnsDomainIs` always includes subdomains. To match `mail.example.com`
and `chat.example.com` but NOT `example.com`, use `*.example.com`.
...
`tinyproxy` runs **inside WSL**, which means it does DNS lookups via WSL's resolver, not via Windows' resolver. Internal hosts pinned to private IPs in your Windows
`C:\Windows\System32\drivers\etc\hosts` file are invisible to WSL by default.
